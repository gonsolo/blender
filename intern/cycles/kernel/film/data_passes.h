                                              ...
                                              ccl_global float *ccl_restrict render_buffer)
{
  ...
  if (!(path_flag & PATH_RAY_SINGLE_PASS_DONE)) {
    if (!(sd->flag & SD_TRANSPARENT) || kernel_data.film.pass_alpha_threshold == 0.0f ||
        average(surface_shader_alpha(kg, sd, closures)) >= kernel_data.film.pass_alpha_threshold) {
      ...
      if (flag & PASSMASK(NORMAL)) {
        const float3 normal = surface_shader_average_normal(kg, sd, closures);
        film_write_pass_float3(buffer + kernel_data.film.pass_normal, normal);
      }
      if (flag & PASSMASK(ROUGHNESS)) {
        const float roughness = surface_shader_average_roughness(closures);
  ...
  if (kernel_data.film.cryptomatte_passes) {
    ...
    const float matte_weight = average(throughput) *
                               (1.0f - average(surface_shader_transparency(kg, sd, closures)));
  ...
  if (flag & PASSMASK(MIST)) {
    ...
    const Spectrum throughput = INTEGRATOR_STATE(state, path, throughput);
    const Spectrum alpha = surface_shader_alpha(kg, sd, closures);
